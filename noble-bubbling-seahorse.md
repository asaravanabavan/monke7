...
│   └── quests.py                      # Quest system (Nemotron-generated quests → structures)
...
## Phase 1.5 (Implemented): Quest System

### Overview
The quest system uses Nemotron to randomly generate contextual quests that lead players through the world. Quests guide players to dungeons, villages, temples, castles, and other structures with objectives and rewards that scale with difficulty.

### New File: `backend/quests.py`
- **Quest models**: `QuestType` (kill, explore, collect, escort, build, survive, clear_dungeon), `QuestStatus` (available, active, completed, failed, expired), `QuestObjective`, `QuestReward`, `Quest`, `PlayerQuestLog`
- **`QuestEngine` class**:
  - `generate_quests(player_uuid, player_name, x, y, z)` — Calls Nemotron to create 1-3 quests based on player stats, playstyle, narrative context, and theme
  - `accept_quest(player_uuid, quest_id)` — Mark a quest as active
  - `update_progress(player_uuid, objective_type, target, count)` — Track objective progress (e.g., mob killed, item collected), auto-completes quests when all objectives are met
  - `get_active_quests(player_uuid)` — List current quests
  - `fail_quest(player_uuid, quest_id)` — Mark quest as failed
  - Fallback quest templates per playstyle (aggressive, explorer, builder, balanced) and narrative theme (retribution, discovery, invasion, mystery) when Nemotron is unavailable
- **Persistence**: `{player_uuid}_quests.json` in `player_data/`
- **Limits**: Max 3 active quests per player, 2-hour expiry on unclaimed quests

### Updated: `backend/nemotron.py`
- New `generate_quests()` function with a dedicated quest system prompt
- Calls Nemotron with player context (playstyle, intensity, narrative theme, recent completed quest names)
- Returns structured quest data with: name, description, giver NPC, objectives, rewards, destination_type, destination_prompt
- Sanitizes AI output: validates entity/item IDs, clamps target counts, falls back gracefully
- Temperature 0.9 for high variety, max 4096 tokens

### Updated: `backend/main.py` — New Endpoints
| Method | Path | Request Model | Purpose |
|--------|------|---------------|---------|
| `POST` | `/api/quests/generate` | `QuestGenerateRequest(player_uuid, player_name, x, y, z)` | Generate new quests via Nemotron |
| `GET` | `/api/quests/{player_uuid}` | Path param | List active/available quests |
| `POST` | `/api/quests/accept` | `QuestAcceptRequest(player_uuid, quest_id)` | Accept a quest |
| `POST` | `/api/quests/progress` | `QuestProgressRequest(player_uuid, objective_type, target, count)` | Update objective progress |
| `POST` | `/api/quests/complete` | `QuestCompleteRequest(player_uuid, quest_id, x, y, z)` | Complete quest & build reward structure |

The `/api/quests/complete` endpoint triggers the full structure generation pipeline using the quest's `destination_prompt`, building the quest destination at the player's location.

### Updated: `backend/narrative.py`
- `NarrativeContext` now includes `active_quest_names` and `active_quest_destinations` fields
- `build_context()` reads the player's quest log to provide quest continuity in narrative hooks
- Quest destinations influence which structure types get generated during auto-generation

### Quest Flow
1. Player stats are tracked via `/api/stats` (existing)
2. Mod calls `POST /api/quests/generate` periodically or on player request
3. Nemotron generates quests based on player's playstyle, intensity, and narrative theme
4. Player accepts a quest via `POST /api/quests/accept`
5. As the player plays, the mod reports progress via `POST /api/quests/progress` (mob kills, item pickups, location arrivals)
6. When all objectives are met, the quest auto-completes and the mod calls `POST /api/quests/complete`
7. The quest's destination structure is built at the player's location as the final reward

### Quest Types and Destinations
| Quest Type | Example Objective | Leads To |
|-----------|-------------------|----------|
| Kill | "Slay 10 zombies in the crypt" | Dungeon, Arena |
| Explore | "Find the Lost Temple" | Temple, Ruins, Village |
| Collect | "Gather 5 ender pearls" | Temple, Mine |
| Escort | "Protect the villagers" | Village, Fort |
| Build | "Place 50 stone bricks to restore the castle" | Castle, Ruins |
| Survive | "Survive the arena waves" | Arena |
| Clear Dungeon | "Destroy all spawners" | Dungeon, Mine |

### Mod-Side Integration (To-Do)
- Add `QuestTracker.java` to display active quests in the action bar / chat
- Call `/api/quests/generate` every 20 minutes or on `/nemo quest` command
- Track mob kills, item pickups, and location arrivals → `POST /api/quests/progress`
- Display quest completion notifications via `/tellraw`
- Call `/api/quests/complete` to build the reward structure
